...
*   **Saila Intake**: Multi-tenant, keyword-triggered, sequential free-text Q&A WhatsApp lead-qualifier built additively on top of Saila.AI. Rapid-reply ordering safeguards (Task #124): each session tracks `last_question_sent_at`; inbound WA messages whose `messages[].timestamp` predates that stamp are treated as late replies to the previous question (transcript-only), and any rapid follow-up to an already-answered question is coalesced. A per-lead Postgres advisory lock (session-level, key = `(0x5A11A1, hashtext('saila_intake:'||lead_id))`) acquired inside `withLeadLock` serialises all inbound through `processInboundForIntake` — works across multiple Node processes/instances, not just within one. The tick worker runs every 15s, so per-question `silence_timeout_seconds` values as low as ~30s are honoured (UI enforces a 30s minimum). Admins define industry-agnostic Intake Flows (`saila_intake_flows`) with trigger keywords (`saila_intake_triggers`) and ordered questions (`saila_intake_questions`); each question targets a `lead.custom_fields` key. Sessions (`saila_intake_sessions`) track depth, fallback attempts, and status (active/paused/completed/abandoned). Engine precedence inside `generateSailaResponse`: (1) intake continuation/start → short-circuits remaining pipeline; (2) Fixed Reply Mode; (3) keyword fast-path / templates / LLM. Drop-off at any question = qualifier signal — depth_reached is logged on the lead. Background tick worker (15s interval, `server/saila-intake-scheduler.ts`) handles silence timeouts, configurable fallback prompts (`{question}` placeholder), and abandonment after max attempts. Optional Sarvam relevance check per question (fail-open). Human takeover detected via `whatsapp_message_logs.origin` column (bot/human) — every human send pauses the active session for 24h. Cancel keywords abort. All bot/human messages mirrored into `lead_updates` for full transcript visibility. Admin UI lives in Saila → "Intake" tab.
...
*   **Saila Broadcast**: Admin-only one-shot WhatsApp send to every contact whose 24h session window is open on a chosen business number. Composer supports text, approved templates, and optional media attachments (image / document / video — public https URL plus optional caption / filename) via the new `sendWhatsAppMedia` Wauper helper in `server/saila-engine.ts`. No `{name}` placeholder; no audience filtering in v1. A cooldown selector (Off / 24h / 48h / 72h / 7d) suppresses any contact who already received a broadcast from this company within the chosen window — per-lead human sends never count toward cooldown because their `whatsapp_message_logs.broadcast_id` is NULL. Cooldown values outside the allowed set are rejected with HTTP 400 (no silent coercion). Sends run in the background at ~5 msg/sec; the active broadcast card polls every 2s and the history list polls every 3s while any row is still in-flight, so progress is rehydrated automatically when an admin navigates away and returns or reloads the tab. Each recipient gets the same bookkeeping as the per-lead send dialog (`whatsapp_message_logs` row with `origin='human'` + `broadcast_id`, `lead_updates` row with `update_via='whatsapp_outgoing'`, active Saila Intake session paused for 24h). Crash recovery on boot marks any `running` broadcast older than 1h as `failed`. Lives in Saila → "Broadcast" tab.
