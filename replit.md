...
*   **Radar (Close Monitor Lead View)**: Admin-managed card-based view for monitoring high-priority leads. Admins can add leads to Radar from the spreadsheet grid via right-click context menu or row dropdown ("Add to Radar" / "Remove from Radar"). The Radar page (`/radar`) displays leads in a 4-column card grid with inline-editable fields: Current Status, Next Step, Expected Closure Date, Site Visit Date, Office Visit Date, and Project Details. Regular users can view leads on radar from their accessible sheets (read-only). Blue `Target` icon indicator appears in the first column of spreadsheet rows that are on Radar. DB: `radar_leads` table with cascade deletes. API: `GET/POST /api/radar`, `PATCH/DELETE /api/radar/:id`, `DELETE /api/radar/by-lead/:leadId`, `GET /api/radar/ids`.
...
*   **Saila.AI**: AI-powered WhatsApp response engine using Sarvam LLM, template matching, keyword matching, confidence scoring, and Wauper Session Messaging API for sending messages. Per-phone toggle (with per-channel access token and phone number ID), conversation tracking, and automatic call booking detection. **LLM Test Mode**: When enabled, AI/LLM-generated responses are stored in conversation history for review but NOT sent via WhatsApp — keyword-matched responses always send regardless. Allows admins to calibrate AI responses before going live. Messages with `sent_status: "dry_run"` are shown with dashed amber styling in the conversation viewer.
