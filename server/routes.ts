...
// ============================================================================
// SHARED HELPER: Calculate user pipeline metrics (Projected & Actual Incentive)
// Used by both Conversion Settings and Vision Board for consistent calculations
// ============================================================================
...
interface UserPipelineMetrics {
  ...
  total_projected_incentive: number;  // Sum of middle stages (exclude first & last)
  total_actual_incentive: number;     // Last stage only, date-filtered
}

async function getUserPipelineMetrics(
  companyId: string,
  userId: string,
  ...
  try {
    ...
    // Helper: get incentive for a value
    ...
    // Helper: filter leads by stage trigger
    ...
    // Calculate cascading multipliers for each stage
    // Multiplier for stage[i] = product of EC% from stage[i+1] to last stage
    const cascadingMultipliers = sortedStages.map((_, index) => {
      if (index === totalStages - 1) return 1; // Final stage: no projection
      let multiplier = 1;
    ...
    const stageMetrics: PipelineStageMetric[] = sortedStages.map((stage, index) => {
      ...
      // Actual incentive: only from final stage
      if (isFinalStage) {
    ...
    console.error("Error in getUserPipelineMetrics:", error);
    return null;
  }
}
...
${questionsList}`;
  ...
  // Generate sync report - shows what system values need to be synced to companies
  app.get("/api/admin/system-columns/sync-report", authMiddleware, requireSuperAdmin, async (req: AuthRequest, res) => {
    try {
      ...
      if (systemValues.length === 0) {
        return res.json({
          status: 'dry_run',
          message: 'No system value definitions found. Run seed first.',
          companies: [],
        });
      }
      ...
      for (const company of companies) {
        const companyReport: {
          companyId: string;
          companyName: string;
          toMarkAsSystem: { columnKey: string; value: string; optionId: string }[];
          toCreate: { columnKey: string; value: string }[];
        } = {
        ...
        for (const [columnType, systemValuesList] of Object.entries(systemValuesByType)) {
          ...
          for (const systemValue of systemValuesList) {
            ...
            if (existingOpt) {
              // Value exists - check if it needs to be marked as system
              if (!existingOpt.is_system) {
                companyReport.toMarkAsSystem.push({
                  columnKey: columnType,
                  value: existingOpt.value,
                  optionId: existingOpt.id,
                });
              }
            } else {
              // Value doesn't exist - needs to be created
              companyReport.toCreate.push({
                columnKey: columnType,
                value: systemValue,
              });
            }
        ...
        // Only include companies that need changes
        if (companyReport.toMarkAsSystem.length > 0 || companyReport.toCreate.length > 0) {
          companyReports.push(companyReport);
        }
      }
      
      const totalToMark = companyReports.reduce((sum, c) => sum + c.toMarkAsSystem.length, 0);
      const totalToCreate = companyReports.reduce((sum, c) => sum + c.toCreate.length, 0);
      
      res.json({
        status: 'dry_run',
        systemValueDefinitions: Object.entries(systemValuesByType).map(([type, values]) => ({
          columnType: type,
          values,
          count: values.length,
        })),
        companiesAffected: companyReports.length,
        totalCompanies: companies.length,
        totalToMarkAsSystem: totalToMark,
        totalToCreate: totalToCreate,
        companies: companyReports,
        message: totalToMark + totalToCreate > 0
          ? `Found ${totalToMark} values to mark as system and ${totalToCreate} missing values to create across ${companyReports.length} companies. Call POST /api/admin/system-columns/sync-execute to proceed.`
          : 'All companies are up to date with system values.',
      });
    } catch (error: any) {
  ...
  // Execute sync - marks matching values as system and creates missing ones
  app.post("/api/admin/system-columns/sync-execute", authMiddleware, requireSuperAdmin, async (req: AuthRequest, res) => {
    try {
      const { confirm } = req.body;
      
      if (confirm !== true) {
        return res.status(400).json({ 
          error: "Sync requires confirmation", 
          message: "Send { confirm: true } to execute the sync" 
        });
      }
      ...
      if (systemValues.length === 0) {
        return res.status(400).json({
          error: 'No system value definitions found',
      ...
      for (const company of companies) {
        ...
        if (markedCount > 0 || createdCount > 0) {
          companyResults.push({
            companyId: company.id,
            companyName: company.name,
      ...
      await storage.createAuditLog({
        user_id: req.userId!,
        company_id: null,
        action: "system_values_sync",
        model: "dropdown_option",
        model_id: null,
        payload: { totalMarked, totalCreated, companiesAffected: companyResults.length },
      });
      
      res.json({
        status: 'completed',
        totalMarkedAsSystem: totalMarked,
        totalCreated: totalCreated,
        companiesAffected: companyResults.length,
        companies: companyResults,
        message: `Successfully marked ${totalMarked} values as system and created ${totalCreated} new system values across ${companyResults.length} companies.`,
      });
    } catch (error: any) {
  ...
  app.patch("/api/admin/company/settings", authMiddleware, requireCompanyAdmin, async (req: AuthRequest, res) => {
    try {
      ...
      const allowedFields = ['mobile_card_columns', 'timezone', 'site_visit_config', 'site_visited_config', 'quick_update_fields', 'add_lead_form_fields', 'auto_fill_rules', 'weekly_off_days', 'quality_check_settings', 'final_value_settings'];
...
Respond with ONLY one word: "meaningful" or "not_meaningful"`;
  ...
  app.delete("/api/sheets/:id/dropdowns/:columnKey/:optionId", authMiddleware, async (req: AuthRequest, res) => {
    try {
      ...
      // Check if this is a system value - system values cannot be deleted
      const option = await storage.getDropdownOptionById(req.params.optionId);
      if (option?.is_system) {
        return res.status(403).json({ 
          error: "Cannot delete system value",
          message: "This is a system-defined value that cannot be deleted. System values are managed globally."
        });
      }
  ...
  app.get("/api/vision-board/:visionBoardId/progress", authMiddleware, async (req: AuthRequest, res) => {
    try {
      ...
      let pipelineStages: PipelineStageMetric[] = [];
      ...
      if (req.companyId && req.userId) {
        try {
          ...
          if (metrics) {
            projectedIncentive = metrics.total_projected_incentive;
            actualIncentive = metrics.total_actual_incentive;
            pipelineStages = metrics.stages;
            
            // Calculate projected progress percent against goal
            const totalPipelineIncentive = projectedIncentive + actualIncentive;
            projectedProgressPercent = board.goal_amount > 0 ? (totalPipelineIncentive / board.goal_amount) * 100 : 0;
          }
        } catch (err) {
          console.error("Error fetching incentives from shared helper:", err);
        }
      }
      ...
      res.json({
        ...
        earnings: {
          ...
          projected_incentive: Math.round(projectedIncentive * 100) / 100,
          actual_incentive: Math.round(actualIncentive * 100) / 100,
          projected_progress_percent: Math.min(100, projectedProgressPercent),
        },
        timeline: {
  ...
  // Uses SHARED HELPER for consistent calculations with Conversion Settings
  ...
  // Uses SHARED HELPER for consistent calculations with Vision Board
  // Now includes ALL users (including admins and multi-sheet users)
  app.get("/api/conversion-settings/analytics/by-user", authMiddleware, requireCompanyAdmin, async (req: AuthRequest, res) => {
    try {
      ...
      // Build date filter for actual incentive
      let actualDateFilter: { startDate: Date; endDate: Date } | undefined;
      if (startDate && endDate) {
        actualDateFilter = {
      ...
      // Sort stages for config output
      const sortedStages = [...settings.stages].sort((a, b) => a.stage_number - b.stage_number);

      // Use shared helper for each user (aggregates across ALL their sheets)
      const userMetricsPromises = eligibleUsers.map(async (user) => {
        const metrics = await getUserPipelineMetrics(req.companyId!, user.id, actualDateFilter);
        ...
        return {
          ...
          user_email: user.email,
          user_role: user.role,
          sheet_count: metrics.sheet_ids.length,
          stages: metrics.stages.map(s => ({
            stage_id: s.stage_id,
            stage_number: s.stage_number,
            stage_name: s.stage_name,
            color: s.color,
            expected_percent: s.expected_percent,
            count: s.count,
            value: s.value,
            incentives: s.incentives,
          ...
          total_projected_incentive: metrics.total_projected_incentive,
          total_actual_incentive: metrics.total_actual_incentive,
        };
      });

      const userMetricsResults = await Promise.all(userMetricsPromises);
      const userMetrics = userMetricsResults.filter(m => m !== null);
