...
export const webhook_logs = pgTable('webhook_logs', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  company_id: varchar('company_id').notNull().references(() => companies.id, { onDelete: 'cascade' }),
  webhook_id: varchar('webhook_id').references(() => company_webhooks.id, { onDelete: 'cascade' }),
  sheet_id: varchar('sheet_id').references(() => sheets.id, { onDelete: 'cascade' }),
  payload: json('payload').$type<Record<string, any>>().default({}).notNull(),
...
export const whatsapp_transfer_settings = pgTable('whatsapp_transfer_settings', {
  ...
  enabled: boolean('enabled').notNull().default(true),
  column_key: varchar('column_key', { length: 255 }).notNull().default('status'),
...
export const whatsapp_message_templates = pgTable('whatsapp_message_templates', {
  ...
  approved_template_variable_count: integer('approved_template_variable_count'),
  enabled: boolean('enabled').notNull().default(true),
  created_at: timestamp('created_at').defaultNow().notNull(),
  updated_at: timestamp('updated_at').defaultNow().notNull(),
}, (t) => ({
...
export const whatsapp_message_logs = pgTable('whatsapp_message_logs', {
  ...
  // Broadcast id when this outgoing row was generated by the Saila Broadcast feature.
  // Null for regular per-lead human sends and for incoming rows. The broadcast cooldown
  // filter joins on this column so per-lead human sends never count toward cooldown.
  broadcast_id: varchar('broadcast_id', { length: 64 }),
...
// ============================================================================
// SAILA BROADCAST — one-shot bulk send to session-open contacts on a number
// ============================================================================
export const saila_broadcasts = pgTable('saila_broadcasts', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  company_id: varchar('company_id').notNull().references(() => companies.id, { onDelete: 'cascade' }),
  sent_by_user_id: varchar('sent_by_user_id').references(() => users.id, { onDelete: 'set null' }),
  send_from_phone: varchar('send_from_phone', { length: 30 }).notNull(),
  message_type: varchar('message_type', { length: 20 }).notNull().default('text'), // text | template
  message_text: text('message_text'),
  ...
  // Recipients excluded by the cooldown filter at resolve time.
...
export type SailaBroadcast = typeof saila_broadcasts.$inferSelect;
export type InsertSailaBroadcast = typeof saila_broadcasts.$inferInsert;
export const insertSailaBroadcastSchema = createInsertSchema(saila_broadcasts).omit({
  id: true,
...
export type InsertSailaBroadcastData = z.infer<typeof insertSailaBroadcastSchema>;
